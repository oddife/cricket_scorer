...
class _FakeBallEventRepository implements BallEventRepository {
  ...
  @override Future<BallEvent?> getBySequence(int inningsId, int sequenceNumber) async =>
      events.where((e) => e.inningsId == inningsId && e.sequenceNumber == sequenceNumber).firstOrNull;
  @override Future<void> deleteById(int id) async { events.removeWhere((e) => e.id == id); }
}

void main() {
  test('undo deletes the last ball and recalculates the innings', () async {
    final innings = Innings(
      id: 1,
      matchId: 1,
      inningsNumber: 1,
    ...
    // This test is intentionally focused on the empty-history guard because
    // BallEvent construction belongs to the scoring-engine tests.
    expect(
