...
class DriftCatalogSyncQueueRepository implements CatalogSyncQueueRepository {
  ...
  Future<void> enqueue({
    ...
    await _db.customStatement(
      ...
      ON CONFLICT(sync_id) DO UPDATE SET
        ...
        status = CASE
          WHEN catalog_sync_queue.status = 'synced' THEN 'pending'
          ELSE catalog_sync_queue.status
        END,
        next_attempt_at = CASE
          WHEN catalog_sync_queue.status = 'synced' THEN NULL
          ELSE catalog_sync_queue.next_attempt_at
        END,
        last_error = CASE
          WHEN catalog_sync_queue.status = 'synced' THEN NULL
          ELSE catalog_sync_queue.last_error
        END,
        synced_at = CASE
          WHEN catalog_sync_queue.status = 'synced' THEN NULL
          ELSE catalog_sync_queue.synced_at
        END
      ''',
  ...
  Future<List<CatalogSyncQueueEntry>> getPending({int limit = 100}) async {
    ...
    final rows = await _db.customSelect(
      '''
      SELECT id, sync_id, entity_type, entity_id, status, attempts,
             created_at, next_attempt_at, last_error, synced_at
  ...
  Future<void> markInProgress(String syncId) async {
    ...
    if (updated != 1) {
      throw StateError('Catalog sync queue entry $syncId is not pending.');
    }
  }
  ...
  Future<void> markSynced(String syncId) async {
    ...
    if (updated != 1) {
      throw StateError('Catalog sync queue entry $syncId was not found.');
    }
  }
