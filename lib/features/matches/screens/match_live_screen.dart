...
class _ScoringView extends ConsumerWidget {
            ...
            final batterCard = Card(
              child: Column(children: [
                ...
                ListTile(
                  ...
                  trailing:
                      Text('${nonStriker?.runs ?? 0}  ${nonStriker?.balls ?? 0}'),
                ),
              ]),
            );
                          ...
                          child: Row(children: [
                            ...
                            Column(
                              ...
                              children: [
                                ...
                                Text(
                                    '${s.completedOvers}.${s.legalBallsInCurrentOver} ov • CRR ${s.legalBalls == 0 ? '0.00' : (s.score / (s.legalBalls / s.ballsPerOver)).toStringAsFixed(2)}'),
                              ],
                            ),
                          ]),
                        ),
  ...
  Future<void> _wicketDialog(BuildContext context, WidgetRef ref,
      LiveScoringState data, List matchPlayers) async {
  ...
  Future<void> _pairDialog(BuildContext context, WidgetRef ref,
      LiveScoringState data, List<int> bowlers) async {
        ...
        child: Padding(
          ...
          child: DropdownButtonFormField<int>(
            ...
            decoration: const InputDecoration(
                labelText: 'Current bowler', border: OutlineInputBorder()),
            items: bowlers
                .map((id) =>
                    DropdownMenuItem(value: id, child: Text(name(id))))
                .toList(),
            onChanged: (id) {
...
class _FinalBowlerSelector extends StatelessWidget {
  const _FinalBowlerSelector({
    required this.selected,
    required this.bowlers,
    required this.name,
    required this.onChanged,
  ...
  Widget build(BuildContext context) => Card(
        child: Padding(
          ...
          child: DropdownButtonFormField<int>(
            initialValue: selected,
            decoration: const InputDecoration(
                labelText: 'Final over bowler', border: OutlineInputBorder()),
            items: bowlers
                .map((id) =>
                    DropdownMenuItem(value: id, child: Text(name(id))))
                .toList(),
  ...
  Widget build(BuildContext context) => Card(
        child: ListTile(
          ...
          trailing: OutlinedButton(
              onPressed: onSelect, child: const Text('Select Pair')),
        ),
      );
}
