...
class MatchFullPdfExportService {
    ...
    document.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.SizedBox(height: 14),
          for (final inning in sorted) ...[
            _inningsSection(inning, states[inning.id]!, ballsByInnings[inning.id]!, playerName, matchPlayers),
  ...
  static pw.Widget _inningsSection(Innings inning, InningsState state, List<BallEvent> balls, String Function(int) playerName, List<MatchPlayer> matchPlayers) {
    ...
    return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
      pw.Text('Innings ${inning.inningsNumber} - ${inning.battingTeamId}', style: pw.TextStyle(fontSize: 15, fontWeight: pw.FontWeight.bold)),
      ...
      pw.TableHelper.fromTextArray(
        ...
        data: bowlers.map((b) => [playerName(b.playerId), '${b.legalBalls ~/ inning.ballsPerOver}.${b.legalBalls % inning.ballsPerOver}', '${b.runsConceded}', '${b.wickets}']).toList(),
        cellStyle: const pw.TextStyle(fontSize: 8),
        headerStyle: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold),
      ),
