...
final GoRouter appRouter = GoRouter(
  ...
  routes: [
    ...
    GoRoute(
      ...
      routes: [GoRoute(path: ':teamId', builder: (context, state) => TeamDetailScreen(teamId: int.parse(state.pathParameters['teamId']!)) )],
    ),
    GoRoute(path: '/players', builder: (context, state) => const PlayerListScreen()),
    GoRoute(
      path: '/tournaments',
      builder: (context, state) => const TournamentListScreen(),
      routes: [GoRoute(path: 'new', builder: (context, state) => const TournamentSetupScreen())],
    ),
  ],
);
