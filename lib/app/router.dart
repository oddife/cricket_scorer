...
import '../features/tournaments/screens/tournament_management_screen.dart';
...
import '../features/matches/screens/live_matches_screen.dart';
...
final GoRouter appRouter = GoRouter(
  ...
  routes: [
    ...
    GoRoute(
      ...
      routes: [
        GoRoute(path: ':teamId', builder: (context, state) => TeamProfileScreen(teamId: int.parse(state.pathParameters['teamId']!)), routes: [
          GoRoute(path: 'manage', builder: (context, state) => TeamDetailScreen(teamId: int.parse(state.pathParameters['teamId']!))),
        ]),
      ],
    ),
    GoRoute(
      ...
      routes: [GoRoute(path: ':playerId', builder: (context, state) => PlayerProfileScreen(playerId: int.parse(state.pathParameters['playerId']!)))] ,
    ),
    GoRoute(
      path: '/tournaments',
      builder: (context, state) => const TournamentListScreen(),
      routes: [
        GoRoute(path: 'new', builder: (context, state) => const TournamentSetupScreen()),
        GoRoute(path: ':tournamentId', builder: (context, state) => TournamentProfileScreen(tournamentId: int.parse(state.pathParameters['tournamentId']!)), routes: [
          GoRoute(path: 'manage', builder: (context, state) => TournamentManagementScreen(tournamentId: int.parse(state.pathParameters['tournamentId']!))),
        ]),
      ],
    ),
  ],
