          ...
          return innings.when(
            ...
            data: (list) => teams.when(
              ...
              data: (teamList) => players.when(
                ...
                data: (playerList) => _ScorecardBody(
                  match: m,
                  innings: list,
                  teams: teamList,
                  players: playerList,
...
class _ScorecardBody extends ConsumerWidget {
  ...
  String playerName(int id) => players
  ...
  Future<_ScorecardData> _load(WidgetRef ref) async {
    ...
    for (final inning in sorted) {
      ...
      final state = const InningsRecalculationEngine().recalculate(
    ...
    return _ScorecardData(rows: rows, result: result);
  }
    ...
    return FutureBuilder<_ScorecardData>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return Center(child: Text('Unable to build scorecard: ${snapshot.error}'));
        }
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        child: Padding(
                          ...
                          child: result.isTie
                              ? const Column(
                                  ...
                                  children: [
                                    Text('MATCH TIED', style: TextStyle(fontWeight: FontWeight.bold)),
                                    SizedBox(height: 6),
                                    Text('Both teams finished with the same aggregate score.'),
                                  ],
                                )
                              : Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    ...
                                    Text(
                                      '${teamName(result.winnerTeamId!)} won',
                                      style: Theme.of(context).textTheme.headlineSmall,
                                    ),
                                    if (result.marginRuns != null)
                                      Text('By ${result.marginRuns} runs'),
                                  ],
                                ),
                        ),
                      ),
                    ...
                    const SizedBox(height: 12),
                    for (final row in rows) _InningsCard(
                      row: row,
                      teamName: teamName(row.innings.battingTeamId),
...
class _ScorecardData {
  const _ScorecardData({required this.rows, required this.result});
  final List<_InningsRow> rows;
  final MatchResult result;
}

class _InningsRow {
  const _InningsRow({required this.innings, required this.state, this.target});
...
class _InningsCard extends StatelessWidget {
  ...
  final _InningsRow row;
  final String teamName;
  final String Function(int) playerName;
  final List<_InningsRow> previousRows;
  ...
  String _leadDeficit() {
    ...
    for (final previous in previousRows) {
      ...
      totals[previous.innings.battingTeamId] =
          (totals[previous.innings.battingTeamId] ?? 0) + previous.state.score;
    }
    ...
    return difference > 0
        ? '$teamName lead by $difference'
        : '$teamName trail by ${difference.abs()}';
  }
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: Text(
                    'Innings ${row.innings.inningsNumber} • $teamName',
            ...
            if (showTarget) ...[
              ...
              if (state.score < row.target!)
                Text('Need ${row.target! - state.score} more'),
              if (state.score >= row.target!) const Text('Target reached'),
            ],
            if (leadDeficit.isNotEmpty) ...[
              const SizedBox(height: 8),
              Text(
                leadDeficit,
                style: const TextStyle(fontWeight: FontWeight.bold),
            ...
            if (batters.isNotEmpty) ...[
              ...
              const Row(
                children: [
                  ...
                  SizedBox(width: 55, child: Text('R')),
                  SizedBox(width: 55, child: Text('B')),
                  SizedBox(width: 55, child: Text('4s')),
                  SizedBox(width: 55, child: Text('6s')),
                ],
              ),
              const SizedBox(height: 6),
