-- Permanent catalog deletion is intentionally available to the app without login.
-- The app does not use Supabase Auth, so the RPC must not require auth.uid().
-- Deletion remains explicit and irreversible, while match-history references
-- still protect players and teams from deletion.
-- 0012 keeps durable tombstones for cross-device deletion synchronization.
