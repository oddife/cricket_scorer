...
class _FakeInningsRepository implements InningsRepository {
  ...
  @override
  Future<Innings?> getById(int inningsId) async => innings;
  @override
  Future<Innings?> getByMatchAndNumber(int matchId, int inningsNumber) async => innings;
  @override
  Future<List<Innings>> getForMatch(int matchId) async => [innings];
  @override
  Future<Innings> create(Innings value) async => value;
  @override
  Future<void> update(Innings value) async {}
}

class _FakeBallEventRepository implements BallEventRepository {
  _FakeBallEventRepository([List<BallEvent>? initial])
      : events = [...?initial];
  ...
  Future<BallEvent> create(BallEvent event) async {
    final persisted = event.copyWith(id: events.length + 1);
...
DeliveryInput _legal([int runs = 0]) => DeliveryInput(
      deliveryType: DeliveryType.normal,
      batterRuns: runs,
    );

DeliveryInput _wide() => const DeliveryInput(
      deliveryType: DeliveryType.wide,
      wideRuns: 1,
    );

DeliveryInput _noBall() => const DeliveryInput(
...
void main() {
  test('normal mode keeps one bowler for an over and requires a new bowler', () async {
    ...
    var result = await service.apply(
      inningsId: 1,
      input: _legal(),
      bowlerId: 20,
      eligibleBowlerIds: const [20, 21],
    );
    ...
    final next = await service.apply(
      inningsId: 1,
      input: _legal(),
      bowlerId: 21,
      eligibleBowlerIds: const [20, 21],
    ...
    expect(repository.events, hasLength(7));
  });
  ...
  test('wide and no-ball do not consume legal balls', () async {
    ...
    final wide = await service.apply(
      inningsId: 1,
      input: _wide(),
      bowlerId: 20,
      eligibleBowlerIds: const [20, 21],
    ...
    expect(wide.rotation.legalBallsInCurrentOver, 0);
    expect(wide.rotation.currentBowlerId, 20);

    final noBall = await service.apply(
      inningsId: 1,
      input: _noBall(),
      bowlerId: 20,
      eligibleBowlerIds: const [20, 21],
    ...
    expect(noBall.rotation.currentBowlerId, 20);
  ...
  test('2-Bowler Mode rejects a bowler outside the active pair', () async {
    ...
    expect(
      () => service.apply(
        ...
        input: _legal(),
        bowlerId: 22,
        eligibleBowlerIds: const [20, 21, 22],
        activeTwoBowlerIds: const [20, 21],
      ),
      throwsA(isA<ArgumentError>()),
    );
  ...
  test('application result contains persisted event, recalculated state and rotation', () async {
    final repository = _FakeBallEventRepository();
    final service = _service(_innings(), repository);
    ...
    expect(result.state.batterRuns[10], 4);
    expect(result.rotation.currentBowlerId, 20);
  });
}
