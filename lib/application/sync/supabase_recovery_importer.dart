...
class SupabaseRecoveryImporter {
  ...
  Future<int> importMatch(RemoteMatchSnapshot snapshot) {
    return _db.transaction(() async {
      ...
      for (final row in snapshot.innings) {
        ...
        inningsIds[syncId] = await _importInnings(
          row,
          matchId,
          teamSourceToSync,
          playerSourceToSync,
  ...
  Future<int> _importMatch(
    ...
    final id = await _db.into(_db.matches).insert(MatchesCompanion.insert(
  ...
  Future<void> _importMatchTeams(List<Map<String, dynamic>> rows, int matchId, Map<String, int> teams) async {
    for (final row in rows) {
      ...
      if (existing != null) {
        ...
        await _db.into(_db.matchTeams).insert(MatchTeamsCompanion.insert(matchId: matchId, teamId: teamId, slot: slot));
      }
    }
  }
  ...
  Future<void> _importMatchPlayers(List<Map<String, dynamic>> rows, int matchId, Map<String, int> teams, Map<String, int> players) async {
    for (final row in rows) {
      ...
      if (existing != null) {
        ...
        await _db.into(_db.matchPlayers).insert(MatchPlayersCompanion.insert(
          matchId: matchId,
          teamId: teamId,
          playerId: playerId,
  ...
  Future<int> _importInnings(
    ...
    final id = await _db.into(_db.innings).insert(InningsCompanion.insert(
      matchId: matchId,
  ...
  Future<void> _importBallEvent(
    ...
    final id = await _db.into(_db.ballEvents).insert(BallEventsCompanion.insert(
      inningsId: inningsId,
      ...
      timestamp: DateTime.parse(_required(row, 'event_timestamp')).toLocal(),
    ));
  ...
  Future<void> _compareBall(
    ...
    if (row['wicket_type'] != null) {
      ...
      _eq('ball wicket completed_runs', context.data['completed_runs'], row['wicket_completed_runs'] ?? 0);
      _eq('ball wicket crossed_before_wicket', context.data['crossed_before_wicket'] != 0, _bool(row['wicket_crossed_before_wicket']));
      _eq('ball wicket replacement_batter_id', context.data['replacement_batter_id'], _nullableSourcePlayer(row['replacement_batter_id'], row, playerSourceToSync, players));
    }
  }
  ...
  Map<String, String> _sourceMap(List<Map<String, dynamic>> rows) {
    return {
      for (final row in rows)
        _sourceKey(row['source_installation_id'], row['local_id']): _required(row, 'sync_id'),
    };
  }
  ...
  int? _nullableSourcePlayer(dynamic remoteLocalId, Map<String, dynamic> row, Map<String, String> sourceToSync, Map<String, int> localBySync) {
    if (remoteLocalId == null) return null;
    return _resolveSourceLocal(localBySync, sourceToSync, remoteLocalId, row, 'player');
  }
  ...
  void _eq(String field, Object? local, Object? remote) {
    if (local != remote) throw StateError('Recovery divergence: $field local=$local remote=$remote');
  }
  ...
  void _validateSnapshot(RemoteMatchSnapshot snapshot) {
    ...
    final expected = snapshot.match['innings_count'] as int;
    if (snapshot.innings.length > expected) throw StateError('Recovery snapshot contains too many innings.');
  }
}
